...
$errors = [
  400 => [
    ...
    "en" => "bad request",
    "message" => [
      "es" => "El servidor no interpreta la solicitud por sintaxis inválida.",
      "en" => "The server cannot interpret the request due to invalid syntax.",
    ],
  ],
  401 => [
    "es" => "no autorizado",
    "en" => "unauthorized",
    "message" => [
      "es" => "Es necesario autenticar para obtener una respuesta.",
      "en" => "Authentication is required to obtain a response.",
    ],
  ],
  403 => [
    "es" => "acceso prohibido",
    "en" => "forbidden",
    "message" => [
      "es" => "Usted no tiene permisos para acceder a este recurso.",
  ...
  404 => [
    ...
    "en" => "page not found",
    "message" => [
      "es" => "No podemos encontrar el recurso que busca.",
      "en" => "We cannot find the resource you are looking for.",
    ],
  ],
  500 => [
    "es" => "interno del servidor",
    "en" => "internal error",
    "message" => [
      "es" => "El servidor está en un estado que no puede manejar.",
      "en" => "The server is in a state it cannot handle.",
    ],
  ],
  502 => [
    ...
    "en" => "bad gateway",
    "message" => [
      "es" => "El servidor obtuvo una respuesta inválida.",
      "en" => "The server received an invalid response.",
    ],
  ],
  503 => [
    ...
    "message" => [
      "es" => "El servidor no está listo para manejar la petición.",
  ...
  504 => [
    ...
    "en" => "gateway timeout",
    "message" => [
      "es" => "El servidor no puede obtener una respuesta a tiempo.",
      "en" => "The server cannot get a response in time.",
    ],
  ],
];
...
if (isset($_POST["custom_error_message"]) && is_string($_POST["custom_error_message"])) {
  $customMessage = $_POST["custom_error_message"];
} elseif (isset($_GET["custom_message"]) && is_string($_GET["custom_message"])) {
  $customMessage = $_GET["custom_message"];
}

$error = $errors[$err];
$back = $lang === "en" ? "Go back" : "Volver";
$sorry = $lang === "en" ? "Sorry for the inconvenience." : "Lamentamos las molestias.";
